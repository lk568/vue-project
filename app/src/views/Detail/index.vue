...
export default {
  ...
  computed: {
    ...
    // 给子组件的数据
    skuImageList() {
      // 如果服务器数据没有回来，skuInfo是一个空对象 那么skuInfo.skuImageList就是undefined；这样不行，要让其为空数组[] 因为上面要用到这个数组中的数据
      return this.skuInfo.skuImageList || [];
    },
  },
  methods: {
    // 改变选择的类型：排他思想，arr是spuspuSaleAttr.spuSaleAttrValueList  spuSaleAttrValue是点击的那个元素
    changeSpuSaleAttrValue(spuSaleAttrValue, arr) {
      // 先清除所有
      arr.forEach((item) => {
        item.isChecked = "0";
      });
